"""
[사용법] python scripts/check_justice_pool.py
scripts/justice_pool.json 과 justice/posts.json 전체를 훑어서
...
def main():
    ...
    for p in posts:
        label = f"[posts.json] {p.get('id', '?')} - {p.get('target', {}).get('ko', '?')}"
        url_to_items[p.get("imageBeforeUrl")].append(label)

    for p in pool:
        label = f"[pool] {p.get('target', {}).get('ko', '?')} - {p.get('title', {}).get('ko', '')[:20]}"
        url_to_items[p.get("imageBeforeUrl")].append(label)
    ...
    print(f"\n[INFO] justice/posts.json 게시글 수: {len(posts)}개")
    print(f"[INFO] scripts/justice_pool.json 남은 글감 수: {len(pool)}개")
    if len(pool) <= 3:
